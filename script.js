...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    yearSelect.addEventListener('change', async () => {
        ...
        if (year) {
            ...
            const meetings = await fetch(`/api/v1/meetings?year=${year}`).then(res => res.json());
    ...
    document.getElementById('grandPrix').addEventListener('change', async (e) => {
        ...
        if (meetingKey) {
            ...
            const sessions = await fetch(`/api/v1/sessions?meeting_key=${meetingKey}`).then(res => res.json());
    ...
    document.getElementById('session').addEventListener('change', async (e) => {
        ...
        if (sessionKey) {
            ...
            const drivers = await fetch(`/api/v1/drivers?session_key=${sessionKey}`).then(res => res.json());
            ...
            const sessionData = await fetch(`/api/v1/sessions?session_key=${sessionKey}`).then(res => res.json());
...
async function visualizeFastestLap() {
    ...
    const stintsUrl = `/api/v1/stints?session_key=${sessionKey}`;
    ...
    const lapsUrl = `/api/v1/laps?session_key=${sessionKey}`;
